...
/**
 * Actors representing a background 
 */
public class Foreground extends Entity {
    ...
    public Foreground(Area area) {
        ...
        sprite = new ImageGraphics(ResourcePath.getForegrounds(area.getTitle()), area.getWidth(), area.getHeight(), null, Vector.ZERO, 1.0f, 0f);
    ...
    public Foreground(Area area, RegionOfInterest region) {
        super(DiscreteCoordinates.ORIGIN.toVector());
        sprite = new ImageGraphics(ResourcePath.getForegrounds(area.getTitle()), area.getWidth(), area.getHeight(), region, Vector.ZERO, 1.0f, 0f);
        sprite.setParent(this);
    }
     ...
     * @param area (Area): ownerArea. Not null
     * @param region (RegionOfInterest): region of interest in the image for the foreground, may be null
     * @param name (String): v file name (i.e only the name, with neither path, nor file extension). Not null
     */
    public Foreground(Area area, RegionOfInterest region, String name) {
        super(DiscreteCoordinates.ORIGIN.toVector());
        sprite = new ImageGraphics(ResourcePath.getForegrounds(name), area.getWidth(), area.getHeight(), region, Vector.ZERO, 1.0f, 0f);
        sprite.setParent(this);
    }
    ...
    public Foreground(String name, int width, int height, RegionOfInterest region) {
        super(DiscreteCoordinates.ORIGIN.toVector());
        sprite = new ImageGraphics(ResourcePath.getForegrounds(name), width, height, region, Vector.ZERO, 1.0f, 0f);
        sprite.setParent(this);
    }
